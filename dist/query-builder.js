...
class QueryBuilder {
  ...
  select(...columns) {
    ...
    if (!columns?.length) {
      ...
      config.parts = config.parts.concat(columns.map(x => {
  ...
  where(...args) {
    ...
    config.sql = config.sql + " " + [resolved[0], resolved.slice(1).join(" and ")].filter(Boolean).join(" and ");
    this.queryConfig.push(config);
    return this;
  }
  column(name, alias) {
  ...
  makeJoin({
    ...
    this.queryConfig.push({
      sql: [type, `${this.schemaStr}${table}${to_alias ? ` as ${to_alias}` : ""}`, "on", this.resolveColumnName(from, from_alias ? `${from_alias}.` : from_alias), operator, this.resolveColumnName(to, to_alias ? `${to_alias}.` : to_alias)].join(" ")
    });
    return this;
  }
  resolveOperator(input, funcName) {
    ...
    if (!!this.inArrayOperator[funcName]) {
      ...
      const sql = [this.resolveOperator(column), this.inArrayOperator[funcName], "(", fArgs.map(x => {
        if (typeof x === "function") {
          const y = x(this);
          return this.resolveOperator(y, y?.type);
        }
  ...
  set(args) {
    const config = {
      type: "set",
      parts: ["set"],
      sql: ["set", Object.entries(args || {}).map(([key, value]) => {
        let column = key;
  ...
  insert(tableName, args) {
    const [columns, values] = Object.entries(args).reduce((acc, [key, value]) => {
      let column = key;
      if (this.driver === "mysql") {
        this.qArgs.push(key);
        column = "??";
    ...
    this.queryConfig.push({
      ...
      sql: [`insert into ${this.schemaStr}${tableName}`, "(", columns.join(","), ")", "values", "(", values.join(","), ")"].join(" ")
    });
    return this;
  }
  delete(tableName) {
