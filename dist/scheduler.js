...
class Scheduler {
  ...
  static schedule({
    ...
    if (!ValidationService.validateNumber({
      value: seconds,
      min: 0.001
    })) {
    ...
    worker.on("error", err => {
      if (log) {
        console.table([`Background Worker Error: Job ${id}`, err.name, err.message]);
      }
    });
    worker.on("exit", exitCode => {
      if (log) {
        console.table([`Background Worker Exited: Job ${id}!`, `Exit Code ${exitCode}`]);
      }
    });
  }
  static getJobs() {
